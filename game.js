...
function createAudioTextures() {
    ...
    // WICHTIG: Stoppe alte Musik-Instanz BEVOR neue erstellt wird
    // Dies verhindert doppelte Musik beim Scene-Restart (z.B. nach GameOver)
    
    // Prüfe auf alte Musik-Instanz in dieser Scene
    if (this.backgroundMusic) {
        try {
            this.backgroundMusic.stop();
        } catch (e) {
            // Musik konnte nicht gestoppt werden
        }
    }
    
    // Zusätzliche Prüfung: Stoppe Musik von allen existierenden Scenes
    if (game && game.scene && game.scene.scenes) {
        game.scene.scenes.forEach(scene => {
            if (scene.backgroundMusic && scene !== this) {
                try {
